#!/bin/bash
# ⚠️ LEGACY / OBSOLETE (2026-06-26) — replaced by scripts/pid_tuner.py (interactive GUI tuner).
#    Kept for reference. SECURITY: the hardcoded Pi sudo password was REMOVED (2026-06-26); `sudo`
#    now prompts interactively. Run on the Pi where you can type the password (or set up askpass).
# Usage: tune.sh KP KI KD  -> edits lino_base_config, builds, flashes, restarts the agent
KP=$1; KI=$2; KD=$3
...
sudo teensy_loader_cli --mcu=TEENSY40 -s -w "$HEX" > /tmp/flash.log 2>&1
grep -q Booting /tmp/flash.log || sudo teensy_loader_cli --mcu=TEENSY40 -w "$HEX" >> /tmp/flash.log 2>&1
