...
## Pinout
See the dedicated sheet **[wiring-pinout.md](wiring-pinout.md)** (all pins in one place).

## Firmware & flashing
- Source and structure: **[firmware/firmware.md](../firmware/firmware.md)**.
- Build/flash on the Pi: **[procedures/running-the-robot.md](../procedures/running-the-robot.md)**.
...
- The current firmware includes **debug** additions (`/debug/*` topics) and an **open-loop mode**
  (`/debug/openloop`) to test the motors without the PID. See [firmware/debug-telemetry.md](../firmware/debug-telemetry.md).
